...
def build_input_path(dataset, connectivity_mode):
    return os.path.join('./data/ldw_data', dataset, connectivity_mode, f'LDW_{dataset}_data.pkl')


def build_output_dir(dataset, connectivity_mode):
    return os.path.join('./data/folds_data', dataset, connectivity_mode)
...
def load_data(dataset, connectivity_mode):
    """
    ...
    """
    input_path = build_input_path(dataset, connectivity_mode)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Prepare fold data for static or dynamic connectivity.")
    parser.add_argument(
        "--dataset",
        choices=["abide", "cobre_static", "cobre_fmri"],
        default="abide",
        help="Dataset name matching the step1 output directory.",
    )
    parser.add_argument(
        "--connectivity-mode",
        choices=["dynamic", "static"],
        default="dynamic",
        help="Connectivity mode to load from step1 output.",
    )
...
def convert2graphs(Corr, Adj, SeqLen, Label):
    ...
    for i in tqdm(range(n_sub), desc="Converting to PyG Graphs"):
        for j in range(n_win):
            ...
            labeL = torch.tensor(int(label), dtype=torch.long) # Label as a long tensor
...
if __name__ == "__main__":
    ...
    all_data, all_adj, labels = load_data(args.dataset, args.connectivity_mode)
    print('Data loaded.')
